...
// against the 1-99 range in isolation, without needing a whole Player
// document — whether a number is *required at all* is a document-level rule
// (see the refine below), not something this field schema alone can decide.
...
const PlayerShape = z.object({
  ...
  // Always 1-99 when present, for every player. Presence is required only
  // for ACTIVE players, by the requireNumberWhenActive refine below — an
  // inactive player may have no number at all. Uniqueness among active
  // players is separately enforced by the DB's unique partial index on
  // { number: 1 } where active: true (KAN-14) — it needs every other active
  // player's document, not just this one.
...
// Requires a number for active players. The 1-99 range itself is already
// enforced by the field (ShirtNumberSchema), so this only checks presence.
function requireNumberWhenActive<
  Schema extends z.ZodType<{ active: boolean; number?: number }>,
>(schema: Schema) {
  return schema.superRefine((data, ctx) => {
    if (data.active && data.number === undefined) {
      ctx.addIssue({
        code: "custom",
